...
;(function(root){
    ...
    _.throttle = function(func, wait, options){
        ...
        return function () {    //节流函数
            var now = _.now()   //首次执行节流函数的时间
            args = arguments
            if(!lastTime && options.leading === false){
                lastTime = now
            }

            //配置了leading，首次执行leading === 1500
    ...
    _.debounce = function(func, wait, immediate){
        ...
        var later = function() {
            ...
            if(last < wait){
                timeOut = setTimeout(later, wait - last)    //wait - last 是为了获取最近点击事件的时间跟第一次wait毫秒后时间的时间差
            } else {
                timeOut = null
                if(!immediate){
                    func.apply(this, args)
    ...
    //判断当前浏览器环境下是否存在bug（当前方法跟原型对象上不可枚举的方法重名，返回false），正常应该返回ture
    var hasEnumbug = ({ toString: null }).propertyIsEnumerable('toString')
    //Object原型对象上的方法
    var ObjectPropList = ['constructor','hasOwnProperty','isPrototypeOf','propertyIsEnumerable','toLocaleString','toString','valueOf']
    ...
    _.keys = function(obj) {
        ...
        if(Object.keys){
            return Object.keys(obj) //Object.keys(obj)把自身对象属性循环出来
        }

        for(prop in obj){   //for in 循环也会把自身对象 + 原型对象上的属性循环出来
            if(_.has(obj, prop)){
                [].push.call(result, prop)
            }
        }

        if(!hasEnumbug){
            //处理bug
            for (let index = 0; index < ObjectPropList.length; index++) {
                prop = ObjectPropList[index];
    ...
    //将对象的属性跟值对调，前提是必须保证object里所有的值都是唯一的且可以序列号成字符串
    //key <=> value
